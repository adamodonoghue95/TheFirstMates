	...
	public void onStatus(Status status) {
		...
		if(status.toString().contains(tbot.HASHTAG)){
			...
			if (playerMap.containsKey(username)) {
				tweet_ID = tbot.reply("@"+username+"\nYou are already participating in a game buddy",tweet_ID);
			}
			else {
				gm = new GameOfPoker(4, username);
				playerGames.put(username, gm);
				playerMap.put(username, 0);
				tweet_ID = tbot.reply(gm.welcomeSection(username), tweet_ID);
		...
		if(status.toString().contains(tbot.BOT_ID) || playerMap.get(username) >= 1 && gm.checkGameState(tbot, tweet_ID)){
			...
			//LEVEL 2: CHECK THE FOLD INPUT AND ASK HOW MUCH THEY WANT TO RAISE
				...
				if(!tweet.contains(errorMsg)){
					tweet_ID = tbot.reply(tweet, tweet_ID);
					setStage(3);
				}
				else {
			...
			//LEVEL 6: CHECK THE FOLD INPUT AND ASK HOW MUCH THEY WANT TO RAISE
				...
				if (gm.noOfPlayers() == 1) { // Only one player left
					...
					tweet_ID = tbot.reply(tweet, tweet_ID);
					tweet_ID = tbot.reply(introStr + "Tweet anything to continue, Tweet #DealMeOutFMPoker to leave game", tweet_ID); 
